...
PROGRAM main
    ...
    INTEGER :: i
    INTEGER :: j
    TYPE(MPI_Datatype) :: BRICK
    TYPE(MPI_Datatype) :: BRICK_RESIZED
    ...
    IF (my_rank .EQ. 0) THEN
        ...
        CALL MPI_Type_vector(ROWS * COLS / 2, 1, 2, MPI_INTEGER, BRICK)

        ! Resize the vector so that we can offset them only by the size of an integer
        CALL MPI_Type_create_resized(BRICK, lb, SIZEOF(i), BRICK_RESIZED)
        CALL MPI_Type_commit(BRICK_RESIZED)
