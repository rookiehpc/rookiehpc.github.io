...
PROGRAM main
    ...
    IF (parent .EQ. MPI_COMM_NULL) THEN
        ...
        program_arguments(1) = ''
        CALL MPI_Comm_spawn(program_name, program_arguments, 1, MPI_INFO_NULL, 0, MPI_COMM_WORLD, child, spawn_error, ierror)
    ELSE
        ! We have been spawned by another MPI process so we do have a parent communicator
        CALL MPI_Comm_get_name(parent, name, name_length, ierror)
