!> @b!> @brief Illustrates how to partition a cartesian topology created with
!> MPI_Cart_create.
...
PROGRAM main
    ...
    CALL MPI_Cart_create(MPI_COMM_WORLD, 2, dims, periods, reorder, cartesian_communicator, ierror)

    ! My rank in the new communicator
    CALL MPI_Comm_rank(cartesian_communicator, my_rank, ierror)

    ! Get my coordinates in the new communicator
    CALL MPI_Cart_coords(cartesian_communicator, my_rank, 2, my_coords, ierror)
    ...
    CALL MPI_Cart_sub(cartesian_communicator, remain_dims, subgrid_communicator, ierror)

    ! Get the ranks of all MPI processes in my subgrid and print it
    CALL MPI_Allgather(my_rank, 1, MPI_INTEGER, subgrid_ranks, 1, MPI_INTEGER, subgrid_communicator, ierror)
