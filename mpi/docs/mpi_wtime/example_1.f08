...
PROGRAM main
    ...
    IF (my_rank == 0) THEN
        ...
        CALL MPI_Send(message, 1, MPI_INTEGER, 1, 0, MPI_COMM_WORLD)
    ELSE
        ! If am not the first MPI process, I receive the message from the previous MPI process first
        CALL MPI_Recv(message, 1, MPI_INTEGER, my_rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE)
        ...
        IF (my_rank .NE. comm_size - 1) THEN
            ...
            CALL MPI_Send(message, 1, MPI_INTEGER, my_rank + 1, 0, MPI_COMM_WORLD)
        END IF
    END IF
