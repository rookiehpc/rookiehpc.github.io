...
PROGRAM main
    ...
    TYPE(MPI_Datatype) :: column_not_resized
    TYPE(MPI_Datatype) :: column_resized
    INTEGER :: my_rank
    ...
    CALL MPI_Type_vector(CELLS_PER_COLUMN, 1, CELLS_PER_ROW, MPI_INTEGER, column_not_resized)

    ! Resize it to make sure it is interleaved when repeated
    lower_bound = 0
    extent = SIZEOF(i)
    CALL MPI_Type_create_resized(column_not_resized, lower_bound, extent, column_resized)
    CALL MPI_Type_commit(column_resized)
    ...
    IF (my_rank == 0) THEN
        ...
        ! Send the column
        CALL MPI_Scatter(full_array, 1, column_resized, my_column, CELLS_PER_COLUMN, MPI_INTEGER, 0, MPI_COMM_WORLD)
    ELSE
        ! Receive the column
        CALL MPI_Scatter(full_array, 1, column_resized, my_column, CELLS_PER_COLUMN, MPI_INTEGER, 0, MPI_COMM_WORLD)
    END IF
