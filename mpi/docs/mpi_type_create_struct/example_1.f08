...
PROGRAM main
    ...
    CALL MPI_Type_create_struct(3, lengths, displacements, types, person_type)
    CALL MPI_Type_commit(person_type)
    ...
    SELECT CASE (my_rank)
        CASE (sender_rank)
            ...
            CALL MPI_Send(buffer, 1, person_type, receiver_rank, 0, MPI_COMM_WORLD)
        CASE (receiver_rank)
            ! Receive the message
            CALL MPI_Recv(buffer, 1, person_type, sender_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE)
