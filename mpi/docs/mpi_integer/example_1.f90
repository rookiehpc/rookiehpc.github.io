...
PROGRAM main
    ...
    SELECT CASE (my_rank)
        CASE (sender_rank)
            ...
            CALL MPI_Ssend(intToSend, 1, MPI_INTEGER, receiver_rank, 0, MPI_COMM_WORLD, ierror)
        CASE (receiver_rank)
            ! Receive the int
            CALL MPI_Recv(intReceived, 1, MPI_INTEGER, sender_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierror)
