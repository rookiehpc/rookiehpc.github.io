...
PROGRAM main
    ...
    INTEGER :: my_rank
    Type(MPI_Comm) :: duplicated_communicator
    ...
    CALL MPI_Comm_dup(MPI_COMM_WORLD, duplicated_communicator)
    ...
    IF (my_rank .EQ. 0) THEN
        CALL MPI_Send(some_message_in_user_code, 1, MPI_INTEGER, 1, 0, MPI_COMM_WORLD)
    ...
    IF (my_rank .EQ. 0) THEN
        CALL MPI_Send(some_message_in_library_code, 1, MPI_INTEGER, 1, 0, duplicated_communicator)
    ...
    IF (my_rank .EQ. 1) THEN
        CALL MPI_Recv(buffer_for_user_message, 1, MPI_INTEGER, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE)
        WRITE(*, '(A,I0,A)') '(MPI process 1) Received ', buffer_for_user_message, ' in user code.'
    END IF

    ! The MPI library can safely issue its MPI_Recv
    IF (my_rank .EQ. 1) THEN
        CALL MPI_Recv(buffer_for_library_message, 1, MPI_INTEGER, 0, 0, duplicated_communicator, MPI_STATUS_IGNORE)
