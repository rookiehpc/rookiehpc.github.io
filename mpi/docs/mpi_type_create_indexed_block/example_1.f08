...
PROGRAM main
    ...
    SELECT CASE (my_rank)
        CASE (sender_rank)
            ! Create the datatype
            displacements = [0, 2, 6, 8]
            CALL MPI_Type_create_hindexed_block(4, 1, displacements, MPI_INTEGER, corner_type)
            CALL MPI_Type_commit(corner_type)
            ...
            CALL MPI_Send(buffer, 1, corner_type, receiver_rank, 0, MPI_COMM_WORLD)
        CASE (receiver_rank)
            ! Receive the message
            CALL MPI_Recv(received, 4, MPI_INTEGER, sender_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE)
