...
/**
 * @brief Illustrate how to communicate an array of unsigned 4-byte ints between
 * 2 MPI processes.
 * @details This application is meant to be run with 2 MPI processes: 1 sender
 * and 1 receiver. The former sends an array of unsigned 4-byte ints to the
...
int main(int argc, char* argv[])
{
	...
	switch(my_rank)
	{
		...
		{
			// Send the ints
			uint32_t intsToSend[2] = { 123456789, 987654321 };
			printf("[MPI process %d] I send unsigned ints: %u and %u.\n", my_rank, intsToSend[0], intsToSend[1]);
			MPI_Ssend(intsToSend, 2, MPI_UINT32_T, RECEIVER, 0, MPI_COMM_WORLD);
			break;
		}
		case RECEIVER:
		{
			// Receive the ints
			uint32_t intsReceived[2];
			MPI_Recv(intsReceived, 2, MPI_UINT32_T, SENDER, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
