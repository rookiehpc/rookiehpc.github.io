...
PROGRAM main    
    ...
    SELECT CASE (my_rank)
        CASE (sender_rank)
            ...
            CALL MPI_Ssend(buffer_sent, 1, MPI_INTEGER, receiver_rank, tag, MPI_COMM_WORLD)
        CASE (receiver_rank)
            ! Receives the message
            CALL MPI_Recv(buffer_received, 1, MPI_INTEGER, sender_rank, MPI_ANY_TAG, MPI_COMM_WORLD, status)
