...
PROGRAM main
    ...
    IF (my_rank .EQ. root_rank) THEN
        ...
        CALL MPI_Gather(MPI_IN_PLACE, 1, MPI_INTEGER, buffer, 1, MPI_INTEGER, root_rank, MPI_COMM_WORLD)
        WRITE(*,'(A,I0,A,I0,A,I0,A,I0,A,I0)') 'Values collected on process ', &
            my_rank, ': ', buffer(0), ', ', buffer(1), ', ', buffer(2), ', ', buffer(3), '.'
    ELSE
        CALL MPI_Gather(my_value, 1, MPI_INTEGER, buffer, 0, MPI_INTEGER, root_rank, MPI_COMM_WORLD)
    END IF
