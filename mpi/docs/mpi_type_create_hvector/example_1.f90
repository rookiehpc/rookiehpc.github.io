!> @brief Illustrates how to create a heterogeneous vector MPI datatype.
...
!>            start of each block: 3 sizeof(INTEGER)
!>          <---------------> <--------------->
!>         |                 |                 |
...
PROGRAM main
    ...
    SELECT CASE (my_rank)
        CASE (sender_rank)
            ! Create the datatype
            CALL MPI_Type_create_hvector(3, 1, 3 * SIZEOF(my_rank), MPI_INTEGER, column_type, ierror)
            CALL MPI_Type_commit(column_type, ierror)
            ...
            CALL MPI_Send(buffer(1,0), 1, column_type, receiver_rank, 0, MPI_COMM_WORLD, ierror)
        CASE (receiver_rank)
            ! Receive the message
            CALL MPI_Recv(received, 3, MPI_INTEGER, sender_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierror)
