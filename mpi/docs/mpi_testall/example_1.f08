...
PROGRAM main
    ...
    IF (my_rank .EQ. 0) THEN
        ...
        CALL MPI_Irecv(buffer(0), 1, MPI_INTEGER, 1, 0, MPI_COMM_WORLD, requests(0))
        CALL MPI_Irecv(buffer(1), 1, MPI_INTEGER, 2, 0, MPI_COMM_WORLD, requests(1))

        ! This MPI_Testall is guaranteed to fail since the corresponding MPI_Ssends have not been issued yet.
        CALL MPI_Testall(2, requests, ready, MPI_STATUSES_IGNORE)
        IF (ready) THEN
    ...
    ELSE
        ...
        CALL MPI_Ssend(message, 1, MPI_INTEGER, 0, 0, MPI_COMM_WORLD)
