!> @brief Illustrates how to use a maxloc reduction operation.
!> @details This application consists in every MPI process sending its value
!> along with its rank. The value sent will be used in the max reduction while
!> the rank will be the locator data. This allows for the locator to represent
!> the rank which has the maximum value found. The result of the reduction will
...
PROGRAM main
    ...
    CALL MPI_Reduce(data_pair, reduction_result, 1, MPI_2INTEGER, MPI_MAXLOC, root_rank, MPI_COMM_WORLD, ierror)

    IF (my_rank == root_rank) THEN
        WRITE(*, '(A,I0,A,I0,A)') 'The maximum value is ', reduction_result(0), &
