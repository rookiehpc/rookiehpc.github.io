...
int main(int argc, char* argv[])
{
	...
	switch(my_rank)
	{
		case SENDER:
		{
			...
			MPI_Ssend(&longLongIntToSend, 1, MPI_LONG_LONG, RECEIVER, 0, MPI_COMM_WORLD);
			break;
		}
		case RECEIVER:
		{
			...
			MPI_Recv(&longLongIntReceived, 1, MPI_LONG_LONG, SENDER, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
