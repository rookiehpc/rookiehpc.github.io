...
PROGRAM main
    ...
    INTEGER :: my_rank
    TYPE(MPI_Datatype) :: triangle_type
    INTEGER :: lengths(0:2)
    ...
    SELECT CASE (my_rank)
        case (sender_rank)
            ...
            CALL MPI_Type_create_hindexed(3, lengths, displacements, MPI_INTEGER, triangle_type)
            CALL MPI_Type_commit(triangle_type)
            ...
            CALL MPI_Send(buffer, 1, triangle_type, receiver_rank, 0, MPI_COMM_WORLD)
        case (receiver_rank)
            ! Receive the message
