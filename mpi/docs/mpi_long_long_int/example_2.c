...
int main(int argc, char* argv[])
{
	...
	switch(my_rank)
	{
		case SENDER:
		{
			...
			MPI_Ssend(longLongIntsToSend, 2, MPI_LONG_LONG_INT, RECEIVER, 0, MPI_COMM_WORLD);
			break;
		}
		case RECEIVER:
		{
			...
			MPI_Recv(longLongIntsReceived, 2, MPI_LONG_LONG_INT, SENDER, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
